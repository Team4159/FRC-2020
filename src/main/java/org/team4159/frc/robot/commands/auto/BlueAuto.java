...
public class BlueAuto extends SequentialCommandGroup {
  public BlueAuto(Drivetrain drivetrain, Arm arm, Intake intake, Turret turret, Shooter shooter) {
    addRequirements(drivetrain);

    addCommands(
      new ParallelCommandGroup(
        new FollowTrajectory(Trajectories.SCRIMMAGE_AUTO, drivetrain),
        new WaitCommand(1).andThen(new ToggleArm(arm).andThen(new RunIntake(5, intake)))
      ),
