...
public class Robot extends TimedRobot {
  ...
  static private double GEARING = 8.48;
  ...
  public WPI_TalonFX setupTalonFX(int port, Sides side, boolean inverted) {
  ...
  public void robotInit() {
    ...
    // create left motor
    WPI_TalonFX leftMotor = setupTalonFX(2, Sides.LEFT, false);
    leftMotor.configSelectedFeedbackSensor(FeedbackDevice.IntegratedSensor);
    SpeedControllerGroup leftGroup = new SpeedControllerGroup(leftMotor, setupTalonFX(3, Sides.RIGHT, false));
    leftEncoderPosition = () -> {
      return leftMotor.getSelectedSensorPosition() * encoderConstant;
    };
    leftEncoderRate = () -> {
      return leftMotor.getSelectedSensorVelocity() * encoderConstant;
    };

    WPI_TalonFX rightMotor = setupTalonFX(0, Sides.RIGHT, false);
    SpeedControllerGroup rightGroup = new SpeedControllerGroup(rightMotor, setupTalonFX(1, Sides.RIGHT, false));
    rightEncoderPosition = () -> {
      return -rightMotor.getSelectedSensorPosition() * encoderConstant;
    };
    rightEncoderRate = () -> {
      return -rightMotor.getSelectedSensorVelocity() * encoderConstant;
    };
